...
def main(myargs):
	...
	if "-k" in myargs:
		## Load Key
		# get user password to decrypt encrypted file containing the Fernet key
	...
	if "-e" in myargs:
		...
		# if metadata has been given and user is not appending, then check all metadata files to see if something has been deleted
		...
		print("metadata written in 'metadata.txt'")
	...
	# else if decrypt
		...
		for f in metadata:
			...
			# decrypt file 
			plain_file = decrypt_file(encr_file_path=myargs["-d"] + "/" + metadata[f]["uuid"], key=fernet_key)

			# write decrypted file to disk
			write_file(file_name=out_folder + f, data=plain_file)
